  ...
  const [serviceCreditsTarget, setServiceCreditsTarget] = useState<string | null>(null);
  const [serviceCreditsState, setServiceCreditsState] = useState<RequestState>('idle');
  const [serviceCreditsResult, setServiceCreditsResult] = useState<any>(null);
  ...
  async function handleSendServiceCredits() {
    ...
    try {
      ...
      <section>
        <h2>Send Service Credits</h2>
        <p>Send service credits to another participant in this room.</p>
        <label htmlFor="chyme-service-credits-target">Recipient</label>
        <select
          id="chyme-service-credits-target"
          value={serviceCreditsTarget || ''}
          onChange={(e) => setServiceCreditsTarget(e.target.value || null)}
        >
          <option value="">Select participant</option>
          {roomParticipants
            .filter((p) => p.userId !== roomState?.participants?.[0]?.userId)
            .map((p) => (
        ...
        <label htmlFor="chyme-service-credits-amount">Amount</label>
        <input
          id="chyme-service-credits-amount"
          type="number"
          min="0.01"
          step="0.01"
        ...
        />
        <label htmlFor="chyme-service-credits-message">Message (optional)</label>
        <input
          id="chyme-service-credits-message"
          value={serviceCreditsMessage}
          onChange={(e) => setServiceCreditsMessage(e.target.value)}
        />
        <button type="button" onClick={() => void handleSendServiceCredits()} disabled={serviceCreditsState === 'loading'}>
          Send Service Credits
        </button>
        {serviceCreditsState === 'success' && serviceCreditsResult ? (
          <div>
            <p>Sent {serviceCreditsResult.amount} credits to {serviceCreditsResult.toUserId}!</p>
          </div>
        ) : null}
      </section>
"use client";
